...
export const getById = (id: string): User | undefined => DB.users.find((user: User) => user.id === id);
export const createUser = (data: { name: string, login: string, password: string }): Partial<User> => {
...
export const updateUser = (dataForUpdate: Partial<User>): Partial<User> | Error => {
